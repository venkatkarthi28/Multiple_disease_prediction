...
if selected == "Home":
    ...
    if home_animation:
        st_lottie(home_animation, height=400, key="home")
    st.markdown(f"""
        ...
        if any(x < 0 for x in input_data) or Glucose > 300 or BloodPressure > 200 or Age > 80:
            ...
            try:
                with st.spinner(t('Analyzing your data...')):
                    ...
                    if prediction[0] == 0:
                        diagnosis = t('You are healthy! Low risk of Diabetes.')
                        st.success(diagnosis)
                    else:
                        diagnosis = t('You have a significant risk of Diabetes.')
                        st.error(diagnosis)
                    
                    # Generate health insights
                    insights = get_health_insights("Diabetes", input_data, prediction[0])
            ...
            # ... (Previous code including Diabetes Prediction) ...
        ...
        if any(x < 0 for x in input_data) or RestingBP > 200 or Cholesterol > 600 or MaxHeartRate > 250 or Age > 80:
            ...
            try:
                with st.spinner(t('Analyzing your data...')):
                    ...
                    if prediction[0] == 0:
                        diagnosis = t('You are healthy! Low risk of Heart Disease.')
                        st.success(diagnosis)
                    else:
                        diagnosis = t('You have a significant risk of Heart Disease.')
                        st.error(diagnosis)
                    
                    # Generate health insights
                    insights = get_health_insights("Heart Disease", input_data, prediction[0])
                    st.subheader(t("health_insights"))
                    for insight in insights:
                        st.write(f"💡 {insight}")
        ...
        if any(x < 0 for x in input_data):
            ...
            try:
                with st.spinner(t('Analyzing your data...')):
                    ...
                    if prediction[0] == 0:
                        diagnosis = t('You are healthy! Low risk of Parkinson’s.')
                        st.success(diagnosis)
                    else:
                        diagnosis = t('You have a significant risk of Parkinson’s Disease.')
                        st.error(diagnosis)
                    
                    # Generate health insights
                    insights = get_health_insights("Parkinsons", input_data, prediction[0])
                    st.subheader(t("health_insights"))
                    for insight in insights:
                        st.write(f"💡 {insight}")
...
# 📌 Footer
# ------------------------------------------------
st.markdown('<div class="footer">© 2025 Health Assistant | Venkatkarthikeyan K | Developed </div>', unsafe_allow_html=True)
